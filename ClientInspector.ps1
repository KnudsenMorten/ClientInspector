...
############################################################################################################################################
    ...
    $Download = (New-Object System.Net.WebClient).DownloadFile("https://raw.githubusercontent.com/KnudsenMorten/ClientInspector/main/ClientInspector-functions.psm1", ".\ClientInspector-functions.psm1")  

    If (Test-Path ".\ClientInspector-functions.psm1")
        {
            Import-module ".\ClientInspector-functions.psm1" -Global -force  -WarningAction SilentlyContinue
        }
    Else
        {
            Exit 1   # Needed functions could not be downloaded and loaded !!
        }
